...
# --- Find closest comps from Global Pricing ---
def find_closest_comps(user_coords):
    all_data['distance'] = all_data['coords'].apply(lambda x: geodesic(user_coords, x).miles)
    closest_2 = all_data.nsmallest(2, 'distance')
    return closest_2['Centre'].tolist()

# --- Find closest coworking using Overpass API ---
...
# --- Pre-compute dataset coordinates once ---
if 'coords' not in all_data.columns:
    all_data['coords'] = all_data['Address'].apply(get_coords)
