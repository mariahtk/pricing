...
# --- Page config ---
st.set_page_config(page_title="Global Pricing Tool", page_icon=":bar_chart:", layout="centered")
...
with st.container():
    ...
    logo = "IWG Logo (1).jpg"  # exact filename in your repo
    if os.path.exists(logo):
        st.image(logo, width=250)

    st.markdown('<div class="login-title">Global Pricing Tool</div>', unsafe_allow_html=True)

    email = st.text_input("Enter your corporate email (@iwgplc.com)", key="email_input")

    st.markdown('</div>', unsafe_allow_html=True)

# --- Authentication check ---
if email:
    if email.endswith("@iwgplc.com"):
        st.success(f"Logged in as {email}")
        authenticated = True
    else:
        st.error("Please use a valid @iwgplc.com email.")
        authenticated = False
else:
    authenticated = False

# --- Only load rest of app if authenticated ---
if authenticated:
